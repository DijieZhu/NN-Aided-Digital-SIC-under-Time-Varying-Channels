...
M = 13;            % Memory length for polynomial SIC
...
%% Analog SIC
[~, strongest_path_index] = max([10^-5.57, 10^-5.77, 10^-6.17]); % Example values
strongest_delay_samples = [12.5, 25, 37.5];
strongest_gain = [10^-5.57, 10^-5.77, 10^-6.17];
strongest_gain = strongest_gain(strongest_path_index);
strongest_delay_samples = round(strongest_delay_samples(strongest_path_index) * fs / 1e9);

analog_residual_frames = zeros(size(rxdata_frames));

for frame_idx = 1:num_frames
    % Perform analog SIC for each frame
    ...
    analog_residual_frames(:, frame_idx) = rxdata_frame - cancellation_signal;
end

%% Linear Cancellation (Least Squares)

mest = M; % Memory length used in LS estimation
ls_frame_size = 500; % Only use 200 samples for LS estimation
...
layers = [
    ...
    reluLayer('Name', 'ReLU2') % Nonlinear activation
...
mean_eval_SIC = mean(C_dB_Eval);
var_eval_SIC = var(C_dB_Eval);

fprintf('Evaluation Frames - Mean SIC: %.2f dB, Variance: %.2f dB\n', mean_eval_SIC, var_eval_SIC);
